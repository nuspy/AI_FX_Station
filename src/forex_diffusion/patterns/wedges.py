        ...
        for end in range(self.min_span, n):
            for span in range(self.min_span, min(self.max_span, end)+1, 10):
                ...
                if touch >= self.min_touches:
                    ...
                    events.append(PatternEvent(self.key,"chart",direction, ts.iloc[start], ts.iloc[end], "confirmed", 0.53, float(a[end]), touch, span, None, span//2, {"upper_line":(start,end,float(s_hi),float(b_hi)),"lower_line":(start,end,float(s_lo),float(b_lo))}))
