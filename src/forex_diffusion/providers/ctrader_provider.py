...
class CTraderProvider(BaseProvider):
    ...
    async def _get_historical_bars_impl(
        ...
        try:
            ...
            # Log number of trendbars received
            logger.info(f"[{self.name}] Received {len(response.trendbar)} trendbars from cTrader for {symbol} {timeframe} (requested range: {start_ts_ms}-{end_ts_ms})")
