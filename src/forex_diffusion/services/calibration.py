...
class CalibrationService:
    ...
    def __init__(
        self, engine: Optional[Engine] = None, db_writer: Optional["DBWriter"] = None
    ):
        self.cfg = get_config()
        db_url = (
            getattr(self.cfg.db, "database_url", None)
            or (self.cfg.db.get("database_url") if isinstance(self.cfg.db, dict) else None)
        )
        if engine is None:
            if not db_url:
    ...
    def calibrate_from_history(
        ...
        """
        logger.info(
            "CalibrationService: calibrating {} {} (alpha={}, half_life_days={})",
            symbol,
            timeframe,
            alpha,
            half_life_days,
        ...
        try:
            if self.db_writer is not None:
                ok = self.db_writer.write_calibration_async(
                    symbol=rec.symbol,
                ...
                if not ok:
                    logger.warning(
                        "CalibrationService: DBWriter queue full, falling back to sync persist"
                    )
            ...
            logger.exception(
                "CalibrationService: failed to persist calibration record: {}", e
            )
    ...
    def _persist_record(self, rec: CalibrationRecord) -> None:
        try:
            ...
            logger.info(
                "Calibration record persisted for {}/{}", rec.symbol, rec.timeframe
            )
        except Exception as e:
    ...
    def get_last_calibration(
        ...
        with self.engine.connect() as conn:
            stmt = (
                self.tbl.select()
                .where(self.tbl.c.symbol == symbol)
                .where(self.tbl.c.timeframe == timeframe)
                .order_by(self.tbl.c.ts_created_ms.desc())
                .limit(1)
            )
