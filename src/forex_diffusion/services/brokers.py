# src/forex_diffusion/services/brokers.py
...
class PaperBroker(BrokerServiceBase):
    ...
    def place_order(self, order: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        o = dict(order)
        o["id"] = oid
        o["time"] = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
        o["status"] = "OPEN"
        self._orders[oid] = o
...
class IBroker(BrokerServiceBase):
    ...
    def place_order(self, order: Dict[str, Any]) -> Tuple[bool, str]:
        logger.warning("IBroker stub in use; delegating to PaperBroker")
        return self._fallback.place_order(order)
...
class MetaTraderBroker(BrokerServiceBase):
    """MetaTrader (MT4/MT5) stub. Integrate via bridge or Python binding in production."""
    def __init__(self):
        self._fallback = PaperBroker()
        self.server = get_setting("mt_server", "")
        self.login = get_setting("mt_login", "")
        self.password = get_setting("mt_password", "")

    def place_order(self, order: Dict[str, Any]) -> Tuple[bool, str]:
        logger.warning("MetaTrader stub in use; delegating to PaperBroker")
        return self._fallback.place_order(order)
...
def get_broker_service() -> BrokerServiceBase:
    """Factory: pick broker from settings, fallback to Paper."""
    mode = str(get_setting("broker_mode", "paper")).lower()
    if mode == "ib":
        return IBroker()
    if mode in ("mt4","mt5","metatrader"):
