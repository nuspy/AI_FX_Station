...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--num_gpus", type=int, default=1,
                    help="Number of GPUs to use (1-N)")
    ap.add_argument("--disable_optimizations", action="store_true",
    ...
    ap.add_argument("--estimate_speedup", action="store_true",
...
def train_worker(
    ...
    # Log training info
    if is_main_process():
        logger.info(f"Starting optimized training on {len(train_loader)} batches (val {len(val_loader)})")
        ...
        logger.info(f"  Baseline (no optimizations): {estimate['baseline_hours']:.2f} hours")
...
def main() -> None:
    ...
    if args.estimate_speedup:
        ...
        print()
        print(f"Baseline time (no optimizations): {estimate['baseline_hours']:.2f} hours")
        ...
        if opt_config.gradient_accumulation_steps > 1:
            print(f"  ✓ Gradient Accumulation ({opt_config.gradient_accumulation_steps} steps)")
        print()
